...
# ETHERNET 0
# ---------------------------------------------------------------------
puts "ETHERNET 0 BEGIN"
# ---------------------------------------------------------------------
# Check all the Ethernet 0 revision and scratch registers
# ---------------------------------------------------------------------
puts "Check ETHERNET_0 version and scratch registers"
foreach eth {ETHERNET_0} {
...
set pcs_status0 [ac7t1500::csr_read_named CSR_SPACE ETHERNET_0 QUAD_PCS_0 BASER_STATUS1_0]
puts "PCS status0 = $pcs_status0"

if { ($pcs_status0) == "00001001" } {
    set first_run 0
    message -info "Ethernet 0 already locked. PCS status $pcs_status0"
} else {
    set first_run 1
    message -warning "Ethernet not yet locked, run initialization. PCS status $pcs_status0"
}
...
if { ![ac7t1500::get_reg_lib_sim_generate] && ($first_run == 1) } {
    ...
    # Set NES loopback.  Serdes near end serial.
    puts "Set Serdes near end serial (NES) Loopback."
    foreach eth {ETHERNET_0} {
